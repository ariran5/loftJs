...
function returnArgumentsArray() {
  return [].slice.call(arguments,0,arguments.length);
}
...
function findAllPSiblings(where) {
    let array = where.children;
    let newArray = [];

    for (var index = 1; index < array.length; index++) {
        if(array[index].tagName === 'P')
...
function deleteTextNodesRecursive(where) {
    ...
    function deleteNodes(where) {
        ...
        for (var index = 0; index < array.length; index++) {
            ...
            if (array[index].nodeType == Node.ELEMENT_NODE) {

                if (array[index].childNodes.length) {
                    deleteNodes(array[index]);
                }
            }
        }
...
function collectDOMStat(root) {
    ...
    deepStat(root)
    function deepStat(tree){
      let array = tree.childNodes;
        ...
        for (var index = 0; index < array.length; index++) {
          ...
          if (array[index].nodeType == Node.ELEMENT_NODE) {
            let tagName = array[index].tagName;
            ...
            for (var q = 0; q < classList.length; q++) {
              stat.classes[classList[q]] = stat.classes[classList[q]] ? ++stat.classes[classList[q]] : 1;
              
            }
            if (array[index].childNodes.length)
              deepStat(array[index]);
          }
